...
public class OwnerRegister extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ownerPic = (ImageView)findViewById(R.id.imageView2);
        ownerPic.setImageResource(R.drawable.owner_image);
        oname = (EditText)findViewById(R.id.name);
        omobile = (EditText)findViewById(R.id.phone);
        oemail = (EditText)findViewById(R.id.mail);
        opassword = (EditText)findViewById(R.id.password);
        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (sname1.isEmpty()||smobile1.isEmpty()||semail1.isEmpty()
                    ...
                    Toasty.error(getApplicationContext(),"Phone Number Wrong",Toasty.LENGTH_SHORT).show();
                }
                else if (spassword1.length()<8){
                    Toasty.error(getApplicationContext(),"Password Size is Small",Toasty.LENGTH_SHORT).show();
                }
                else if(!Patterns.EMAIL_ADDRESS.matcher(semail1).matches()){
                    Toasty.error(getApplicationContext(),"Enter a Vaild E-mail",Toasty.LENGTH_SHORT).show();
                }
                else if(flagEmail==0){
                    ...
                    Toasty.warning(getApplicationContext(),"Upload in Progress",Toasty.LENGTH_SHORT).show();
                }
                else if (imageFlag==0){
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            ...
                            if(task.isSuccessful()){
                                //here add details to database
                                    ...
                                    uploadFile();
                                    OwnerProfile ownerProfile = new OwnerProfile(sname1,smobile1,semail1,spassword1,"owner",imageURL);
                                    myRef.setValue(ownerProfile);
                                    b1.setVisibility(View.INVISIBLE);
    ...
    private void uploadFile(){
        ...
        if(mImageuri!=null){
            final StorageReference fileReference = mstorageReference.child(mAuth.getUid()).child("Images").child("Profile Pic");
            try{
                        ...
                        .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                            ...
                            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                                ...
                                try{
                                    FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance();
                                    DatabaseReference myRef = firebaseDatabase.getReference("Owners/"+mAuth.getUid());
                                    myRef.child("OimageURL").setValue(imageURL);
                                    flag = 1;
                                }
